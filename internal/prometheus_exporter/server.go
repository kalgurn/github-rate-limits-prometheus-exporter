...
func newLimitsCollector() *LimitsCollector {
	return &LimitsCollector{
		LimitTotal: prometheus.NewDesc(prometheus.BuildFQName("github", "rate_limit", "total"),
			"Total limit of requests for the installation",
			nil, prometheus.Labels{
				"account": githubAccount,
			}),
		LimitRemaining: prometheus.NewDesc(prometheus.BuildFQName("github", "rate_limit", "remaining"),
			"Amount of remaining requests for the installation",
			nil, prometheus.Labels{
				"account": githubAccount,
			}),
		LimitUsed: prometheus.NewDesc(prometheus.BuildFQName("github", "rate_limit", "used"),
			"Amount of used requests for the installation",
			nil, prometheus.Labels{
				"account": githubAccount,
			}),
		SecondsLeft: prometheus.NewDesc(prometheus.BuildFQName("github", "rate_limit", "time_left_seconds"),
...
func (collector *LimitsCollector) Collect(ch chan<- prometheus.Metric) {
	...
	limits := github_client.GetCoreResourceLimits(auth.InitClient())
